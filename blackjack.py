...
class Button:
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface,
                         CLR_WHITE if self.enabled else CLR_GRAY,
...
class BlackjackGame:
    ...
    def deal_initial(self):
        ...
        sequence = [
            (True,  True),   # player card 1 — face up
            (False, False),  # dealer card 1 — hole card
            (True,  True),   # player card 2 — face up
    ...
    def resolve(self):
        ...
        if is_bust(self.player_hand):
            result = "BUST"
            sub    = f"Dealer wins \u2013 you lose ${self.bet}"
            gain   = 0
        elif pBJ and dBJ:
            result = "PUSH"
            sub    = "Both Blackjack \u2013 bet returned"
            gain   = self.bet
        elif pBJ:
            win    = int(self.bet * 2.5)
            result = "BLACKJACK!"
            sub    = f"\u2660 Blackjack pays 2.5\u00d7  +${win}"
            gain   = win
        elif dBJ:
            result = "DEALER BLACKJACK"
            sub    = f"Dealer wins \u2013 you lose ${self.bet}"
            gain   = 0
        elif is_bust(self.dealer_hand):
            win    = self.bet * 2
            result = "DEALER BUSTS"
            sub    = f"You win  +${win}"
            gain   = win
            ...
            win    = self.bet * 2
            result = "YOU WIN!"
            sub    = f"{pv} beats {dv} \u2013 +${win}"
            gain   = win
        elif dv > pv:
            result = "DEALER WINS"
            sub    = f"{dv} beats {pv} \u2013 you lose ${self.bet}"
            gain   = 0
        else:
            result = "PUSH"
            sub    = f"Tie at {pv} \u2013 bet returned"
            gain   = self.bet
        ...
        if gain > self.bet or "BLACKJACK" in result:
            for _ in range(90):
                col = random.choice([
                    (212, 175, 55), (255, 215, 0),
                    (255, 255, 100), (200, 150, 0)])
                self.particles.append(
                    Particle(SCREEN_W//2, SCREEN_H//2, col))
        ...
        for i in range(5, 0, -1):
            pygame.draw.rect(self.screen, (25, 25, 105),
                             (self.DECK_POS[0]+i, self.DECK_POS[1]+i,
                              CARD_W, CARD_H), border_radius=CARD_RADIUS)
    ...
    def draw_betting_ui(self, mouse_pos):
        ...
        pygame.draw.rect(self.screen, CLR_GOLD, inp_rect, 2, border_radius=6)
        ...
        hint = fnt_tiny.render(
            "Type amount or click chips  |  ENTER to confirm",
            True, CLR_GRAY)
        ...
        if self.input_error:
            ...
            self.screen.blit(err, (SCREEN_W//2 - err.get_